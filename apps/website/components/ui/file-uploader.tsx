...
export const FileUploader = forwardRef<HTMLDivElement, FileUploaderProps & React.HTMLAttributes<HTMLDivElement>>(
	(
		{ className, dropzoneOptions, value, onValueChange, reSelect, useLOF = true, disabled = false, orientation = 'vertical', children, dir, ...props },
		ref
	) => {
		...
		useEffect(() => {
			if (!value) return;
			if (value.length === maxFiles && useLOF) {
				setIsLOF(true);
				return;
			}
			setIsLOF(false);
...
export const FileUploaderItem = forwardRef<HTMLDivElement, { index: number } & React.HTMLAttributes<HTMLDivElement>>(
	({ className, index, children, ...props }, ref) => {
		...
		return (
			<div
				ref={ref}
				className={cn(buttonVariants({ variant: 'ghost' }), 'relative h-6 cursor-pointer justify-between p-1', className, isSelected ? 'bg-muted' : '')}
				{...props}
			>
...
export const FileInput = forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(({ className, children, ...props }, ref) => {
	...
	return (
		<div ref={ref} {...props} className={`relative w-full ${isLOF || disabled ? 'cursor-not-allowed opacity-50' : 'cursor-pointer'}`}>
			<div
				className={cn(
